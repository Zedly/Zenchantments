...
public class CompatibilityAdapter {
	...
	public final EnumStorage<Material> CROP_YEILDS  =
	...
	public final EnumStorage<Material> SHRED_PICKS = new EnumStorage<>(new Material[]{STONE, GRANITE,
		ANDESITE, DIORITE, NETHERRACK, GLOWSTONE, SANDSTONE, RED_SANDSTONE, ICE, PACKED_ICE, BLUE_ICE}, ORES,
		TERRACOTTAS);

	public final EnumStorage<Material> SHRED_SHOVELS = new EnumStorage<>(new Material[]{GRASS_BLOCK, PODZOL, DIRT,
	...
	public final double[]   GLUTTONY_SATURATIONS = {2.4, 6, 1.2, 7.2, 6, 3.6, 0.2, 7.2, 6, 9.6, 12.8, 6, 9.6, 0.4, 0.6,
		1.2, 7.2, 4.8, 12, 12.8};
	public final Material[] GLUTTONY_FOOD_ITEMS  = new Material[]{
	...
	public final EnumStorage<Biome> DRY_BIOMES = new EnumStorage<>(new Biome[]{Biome.DESERT, Biome.FROZEN_OCEAN,
			...
			if (!evt.isCancelled()) {
				if (target instanceof Sheep) {
					...
					sheep.getLocation().getWorld().dropItem(sheep.getLocation(),
						new ItemStack(WOOL.get(sheep.getColor().ordinal()), RND.nextInt(3) + 1));
