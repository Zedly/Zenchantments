                ...
                case "give":
                    if (args.length == 2) {
                        ...
                        if (ench != null) {
                            ...
                            for (int i = 1; i <= vEnch.getMaxLevel(); i++) {
                                results.add(i + "");
                            }
                        } else {
                            ...
                            for (Enchantment e : Enchantment.values()) {
                                if (e.getName().startsWith(args[args.length - 1]) && e.canEnchantItem(stk)) {
    ...
    private static boolean give(CommandSender sender, String[] args) {
        ...
        if (args.length >= 4) {
            ...
            while (scanner.hasNext()) {
                ...
                if (ench != null) {
                    if (ench.validMaterial(mat) || mat == Material.ENCHANTED_BOOK) {
                    ...
                    if (vEnch.canEnchantItem(stk) && level <= vEnch.getMaxLevel()) {
                        vanillaEnchantsToAdd.put(vEnch, level);
                    } else {
                        sender.sendMessage(Storage.logo + "The enchantment " + ChatColor.DARK_AQUA
                                + vEnch.getName() + ChatColor.AQUA + " cannot be given in this configuration.");
                    }
                } else {
                    sender.sendMessage(Storage.logo + "The enchantment " + ChatColor.DARK_AQUA
            ...
            for (Map.Entry<Enchantment, Integer> ench : vanillaEnchantsToAdd.entrySet()) {
                stk.addEnchantment(ench.getKey(), ench.getValue());
                msgBldr.append(ChatColor.stripColor(ench.getKey().getName()));
                msgBldr.append(", ");
            }
            if (!enchantsToAdd.isEmpty() && !vanillaEnchantsToAdd.isEmpty()) {
