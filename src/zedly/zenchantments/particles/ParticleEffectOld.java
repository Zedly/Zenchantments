...
public enum ParticleEffectOld {
    ...
    private static final Map<String, ParticleEffectOld> NAME_MAP = new HashMap<>();
    private static final Map<Integer, ParticleEffectOld> ID_MAP = new HashMap<>();
    ...
    static {
        for (ParticleEffectOld effect : values()) {
            NAME_MAP.put(effect.name, effect);
            ID_MAP.put(effect.id, effect);
        }
    }

    private ParticleEffectOld(String name, int id, int requiredVersion, ParticleProperty... properties) {
        this.name = name;
        this.id = id;
    ...
    public static ParticleEffectOld fromName(String name) {
        for (Entry<String, ParticleEffectOld> entry : NAME_MAP.entrySet()) {
    ...
    public static ParticleEffectOld fromId(int id) {
        for (Entry<Integer, ParticleEffectOld> entry : ID_MAP.entrySet()) {
            if (entry.getKey() != id) {
                continue;
            }
            return entry.getValue();
    ...
    private static boolean isDataCorrect(ParticleEffectOld effect, ParticleData data) {
        return ((effect == BLOCK_CRACK || effect == BLOCK_DUST) && data instanceof BlockData) || (effect == ITEM_CRACK && data instanceof ItemData);
    }

    private static boolean isColorCorrect(ParticleEffectOld effect, ParticleColor color) {
    ...
    public static final class ParticlePacket {
        ...
        private final ParticleEffectOld effect;
        ...
        public ParticlePacket(ParticleEffectOld effect, float offsetX, float offsetY, float offsetZ, float speed, int amount, boolean longDistance, ParticleData data) throws IllegalArgumentException {
            initialize();
            if (speed < 0) {
        ...
        public ParticlePacket(ParticleEffectOld effect, Vector direction, float speed, boolean longDistance, ParticleData data) throws IllegalArgumentException {
            this(effect, (float) direction.getX(), (float) direction.getY(), (float) direction.getZ(), speed, 0, longDistance, data);
        }

        public ParticlePacket(ParticleEffectOld effect, ParticleColor color, boolean longDistance) {
            this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1, 0, longDistance, null);
            if (effect == ParticleEffectOld.REDSTONE && color instanceof OrdinaryColor && ((OrdinaryColor) color).getRed() == 0) {
            ...
            try {
                ...
                if (version < 8) {
                    ...
                    if (data != null) {
                        ...
                        ReflectionUtils.setValue(packet, true, "k", effect == ParticleEffectOld.ITEM_CRACK ? packetData : new int[]{packetData[0] | (packetData[1] << 12)});
                    }
                }
